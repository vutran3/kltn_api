...
class HealthCheckController {
    ...
    collectImageWeekly = async (req, res, next) => {
        ...
        const tsRaw =
            b.ts ?? b.capturedAt ?? b.timestamp ?? b.time ?? b.t ?? Date.now();
        ...
        if (!Number.isFinite(tsMs)) {
            throw new BadRequestError('Invalid timestamp (ts/capturedAt/timestamp)');
        }
        ...
        const confThreshold = (b.conf_threshold && Number(b.conf_threshold)) || 0.5;
        ...
        const userId =
            req.get('x-user-id') || (req.headers && req.headers['x-user-id']) || 'user001';

        const noAbnormalDetected =
            (predict &&
                predict.is_diseased === false &&
                (predict.num_detections === 0 ||
                    (Array.isArray(predict.boxes) && predict.boxes.length === 0))) ||
