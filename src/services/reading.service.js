        ...
        const rows = await ReadingBucket.aggregate([
            ...
            { $limit: lim },
            {
                $project: {
                    _id: 0,
                    t: "$readings.t",
                    airTemperature: "$readings.air_temperature",
                    airHumidity: "$readings.air_humidity",
                    lightRaw: "$readings.light_raw",
                    soilTemperature: "$readings.soil_temperature",
                    soilHumidity: "$readings.soil_humidity",
